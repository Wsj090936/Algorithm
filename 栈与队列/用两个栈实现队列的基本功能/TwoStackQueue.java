 ...
 * 当需要出队列的时候，将Stackpush中的栈顶元素弹入到Stackpop中，
 * 如果弹入之前Stackpop已经有元素了，就不需要弹入了，直接让Stackpop弹出即可，
 * 即Stackpop中最多只能有一个元素，否则会打乱队列先入先出的顺序
 */
public class TwoStackQueue {
public static void main(String[] args) {
	StackQueue S1 = new StackQueue();
	int[] num = new int[]{1,2,3,4,5,6,7,8,9,0};//测试用例
...
class StackQueue{
	...
	public int poll(){//出队列
		...
		}else if(Stackpop.empty()){//如果Stackpop为空，就可以往里面添加下一个元素，并弹出，若不为空，则直接弹出
